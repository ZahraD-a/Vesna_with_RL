...
/**
 * Fast movement action for RL training (2x speed).
 * Same as walk but sends "run" type to Godot for faster movement.
 *
 * Usage in ASL:
 *   vesna.run(target)       - runs to target at 2x speed
 *   vesna.run(target, id)   - runs to target with id at 2x speed
 */
public class run extends DefaultInternalAction {
    ...
    public Object execute( TransitionSystem ts, Unifier un, Term[] args ) throws Exception {
        ...
        action.put( "type", "run" );  // "run" instead of "walk" for 2x speed
